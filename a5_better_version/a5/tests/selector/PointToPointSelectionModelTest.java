...
class PointToPointSelectionModelTest {
    ...
    //  returned value.
    @DisplayName("GIVEN a model in the SELECTING state with a non-empty selection, " +
            "WHEN a live wire is requested to a new location, " +
            "THEN it will return a straight line segment from the last point of the " +
            "selection to the new location.")
    @Test
    void testLiveWireNonEmpty() {
        SelectionModel model = new PointToPointSelectionModel(false);
        Point startPoint = new Point(0, 0);
        Point secondPoint = new Point(10, 0);
        model.addPoint(startPoint);
        model.addPoint(secondPoint);
        Point mouseLocation = new Point(5, 5);
        PolyLine wire = model.liveWire(mouseLocation);
        PolyLine expectedWire = new PolyLine(secondPoint, mouseLocation);
        assertEquals(expectedWire, wire);
    }
