...
public abstract class SelectionModel {
    ...
    public Point lastPoint() {
        ...
        return selection.isEmpty() ? start : selection.getLast().end();  // Replace this line
    }
    ...
    protected void undoPoint() {
        if (selection.isEmpty()) {
            ...
            LinkedList<PolyLine> oldSelection = new LinkedList<>(selection);
            selection.removeLast();
            if (state == SELECTED && !selection.isEmpty()) {
                // If we remove a point from a closed path, we revert to SELECTING state
                setState(SELECTING);
            }
            propSupport.firePropertyChange("selection", oldSelection, selection());
        }
    }
